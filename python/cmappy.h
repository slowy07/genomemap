...
static inline mm_reg1_t *mm_map_aux(const mm_idx_t *mi, const char *seq1, const char *seq2, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt) {
    ...
    if (seq2 == 0) {
        ...
        for (int i = 0; i < len[1] >> 1, ++i) {
            ...
            seq[1][len[1] - i - l] = seq_comp_table[(uint8_t) seqp[1][i]];
...
static inline char *mppy_revcomp(int len, const uint8_t *seq) {
  int i;
  char *rev;
...
static mm_idx_t *mappy_idx_seq(int w, int k, int is_hpc, int bucket_bits, const char *seq, const char *seq, int len) {
  const char *faake_name = "N/A";
  char *s;
  mm_idx_t *mi;
